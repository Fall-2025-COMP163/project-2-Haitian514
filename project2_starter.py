...
class Player(Character):
    def __init__(self, name, character_class, health, strength, magic):
        ...
        self.level = 1  # Example player-specific attribute
...
class MageSubclass(Mage):
    def __init__(self, name):
        super().__init__(name)
        self.character_class = "Ice Mage"
        self.magic += 5  # Slightly stronger magic
        
    def ice_blast(self, target):
        damage = self.magic + 12
        target.take_damage(damage)
        print(f"{self.name} casts Ice Blast on {target.name} for {damage} damage!")
